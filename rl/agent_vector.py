...
class DQNAgentVector:
    """DQN Agent interacting with environment.
    
    Attribute:
        env (gym.Env): tainshou.env.SubprocVectorEnv
    ...
    def __init__(
        ...
        self.num_train_envs = len(train_envs)
    ...
    def train(self, num_steps: int, plot=True, plotting_interval: int = 200):
        """Train the agent."""
        self.is_test = False
        ...
        while sampled_steps < num_steps:
            act = [action_space.sample() for action_space in self.train_envs.action_space]          # 随机动作
            obs, rew, done, info = self.train_envs.step(act)
            if np.sum(done):
                self.train_envs.reset(np.where(done)[0])
                sampled_steps += self.num_train_envs
