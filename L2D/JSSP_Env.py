    ...
    def step(self, action):
        ...
        if action not in self.partial_sol_sequeence:
            ...
            if action not in self.last_col:
                self.omega[action // self.number_of_machines] += 1      # 某个Job已完成的task数量, 这个omega就是后面的candidate
            else:
                self.mask[action // self.number_of_machines] = 1        # 某个Job已经全部完成

            self.temp1[row, col] = startTime_a + dur_a                  # end time of a, initialized as all zeros
