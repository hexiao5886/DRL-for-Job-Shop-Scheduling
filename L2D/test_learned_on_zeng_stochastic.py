...
def test_(times, machines, N_JOBS_N = 20, N_MACHINES_N = 20):
    dataset = []
...
if __name__ == '__main__':
    ...
    directory = f"figs_no_future_infomation/p{random_rate}cv{cv}_gin_{str(trained_size)}"
    ...
    for instance in all_instances:
        makespan = test(instance=instance, N_JOBS_N=trained_size[0], N_MACHINES_N=trained_size[1])
        ...
        for i in range(n):
            ...
            policy_val = test_(times=times, machines=scheduler.machines, N_JOBS_N=trained_size[0], N_MACHINES_N=trained_size[1])
