...
class Machine:
    ...
    def doable_ops(self):
        ...
        for op in self.remain_ops:
            ...
            if prev_start:
                ...
                if self.delay:      # True indicates that the machine can enter the wait state when it is idle.
                    # each machine's first processing operation should not be a reserved operation
                    if first_op:
                        cond = prev_done
                    else:
                        cond = (prev_done or prev_process)      # prev_done or prev_process, both are called doable
                else:
                    cond = prev_done
    ...
    def doable_ops_no_delay(self):
        ...
        for op in self.remain_ops:
            ...
            if prev_start:
                ...
                prev_done = op.prev_op.node_status == DONE_NODE     # only prev_done is called doable
                if prev_done:
                    doable_ops.append(op)
        return doable_ops
    ...
    def do_processing(self, t):
        ...
        Args:
            t: Current simulation time.
        """
        if self.remaining_time > 0:  # When machine is doing some operation (can be waiting or processing)
